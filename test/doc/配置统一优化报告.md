...
1. **`config/api_keys.json`** - 存储 API keys 和服务配置
2. **`.env`** - 环境变量配置
...
**配置源**：
- ✅ **`config/api_keys.json`** - API keys 和服务配置（可选）
- ✅ **`.env`** - 环境变量配置（推荐）

**配置加载器**：
- ✅ `src/config_loader.py` - 统一的配置加载器
  - 优先从 `.env` 读取
  - 回退到 `config/api_keys.json`
  - 提供默认值和验证
...
配置统一：
├── .env (环境变量 - 推荐)
└── config/api_keys.json (API 配置 - 可选)
...
- ✅ 用户只需要关注 `.env` 和 `api_keys.json` 两个文件
- ✅ 避免配置分散导致的混乱
...
### 4. 向后兼容
- ✅ 现有的 `.env` 和 `api_keys.json` 配置继续有效
- ✅ 不影响现有用户的使用方式
...
|------|------|------|
| `.env` | 环境变量、敏感信息 | ✅ 推荐 |
| `config/api_keys.json` | API 配置、服务配置 | ❌ 可选 |

### 2. 配置加载原则

1. **优先级**：环境变量 > JSON 配置 > 硬编码默认值
2. **验证**：加载后验证配置完整性
3. **错误提示**：明确提示缺失的配置项
4. **默认值**：为所有配置提供合理的默认值
...
### 问题
- ❌ 配置文件分散（`.env`、`api_keys.json`、`test_config.json`）
...
- ✅ 简化代码，直接使用默认值
- ✅ 统一配置管理，只保留 `.env` 和 `api_keys.json`
